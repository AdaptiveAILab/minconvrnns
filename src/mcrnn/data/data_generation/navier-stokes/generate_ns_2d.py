...
def create_mp4(data: np.array):
    ...
    print(f"Animation written to video.mp4")
...
def generate_data(
    ...
    f = 0.1*(torch.sin(f_mul*math.pi*(X+Y))+torch.cos(f_mul*math.pi*(X+Y)))  # 64x64

    # Inputs
    a = torch.zeros(N, s, s)
    # Solutions
    u = torch.zeros(N, record_steps, 1, s, s)
    ...
    attributes = {
        ...
        "viscosity": viscosity,  # Viscosity is 1/Re, where Re is the Reynolds number
        "delta_t": "%.e" % delta_t,
        "simulation T": T,
...
if __name__ == "__main__":
    ...
    parser.add_argument("--delta-t", type=float, default=1e-2,
